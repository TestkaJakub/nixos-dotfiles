...
let
  ...
  # Writes a self-contained HTML file to /tmp and opens it in the browser.
  # The panel talks to pomo via fetch() calls to a tiny Python HTTP server
  # that forwards commands to the pomo binary and returns JSON.
  #
  # The server runs on localhost:19876 and auto-exits after 10 min of inactivity.
  # A lock file prevents duplicate servers.
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .config-field input.changed { border-color: var(--focus-col); }
...
async function poll() {
  ...
  dot.className = 'status-dot ' + (d.css || '');
...
HTMLEOF
    ...
    if [ -f "$LOCK" ]; then
      ...
      if kill -0 "$srv_pid" 2>/dev/null; then
        # Server already up — just open the panel
        ${xdgOpen} "$HTML"
        exit 0
      else
        rm -f "$LOCK"
    ...
    # Write the Python server inline
...
POMO = "${pomo}/bin/pomo"
JQ   = "${jq}"
IDLE_TIMEOUT = 600  # exit after 10 min of no requests
...
def get_state():
    ...
    if status == "idle":
        ...
        parts = status.split()
        # "paused  focus  session 1  04:55 left"
        # "running  focus  session 1  24:00 left"
        if parts[0] == "paused":
            state["css"]     = "paused"
            state["mode"]    = parts[1]
...
# Watchdog: exit after IDLE_TIMEOUT seconds with no requests
import threading
def watchdog():
    while True:
        time.sleep(30)
